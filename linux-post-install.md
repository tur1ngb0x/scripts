# APT
```
$ cat << EOF | sudo tee /etc/apt/apt.conf.d/90betterapt
...
# Network
```
$ sudo mv -fv /etc/NetworkManager/conf.d/default-wifi-powersave-on.conf{,.bak}
$ printf '[connection]\nwifi.powersave = 2\n' | sudo tee /etc/NetworkManager/conf.d/90-wifi-powersave-off.conf
```
...
# Swappiness
```
$ [[ $(sysctl -n vm.swappiness) -lt 10 ]] && printf '\nvm.swappiness = 10\n' | sudo tee /etc/sysctl.d/80-swappiness.conf
```
